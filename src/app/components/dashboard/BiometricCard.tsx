...
export const BiometricCard = ({ chicken }: BiometricCardProps) => {
  ...
  const isHarvestReady = chicken.weight >= 2000; // 2kg
    ...
    <Card className="flex flex-col h-full w-full bg-zinc-950 border-zinc-800 text-zinc-100 overflow-hidden shadow-xl">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-zinc-900/50 border-b border-zinc-800 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono font-bold tracking-tight">ID {chicken.id}</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-6" aria-live="polite">
          ...
          <div className="grid grid-cols-2 gap-3">
             ...
             <div className="relative aspect-square rounded-sm overflow-hidden border border-zinc-800 group bg-zinc-900">
               ...
               <img 
                  ...
                  alt={`Side Profile of ${chicken.id}`} 
                  className="w-full h-full object-cover opacity-70 grayscale group-hover:opacity-100 transition-all duration-500"
               />
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-3 bg-zinc-900/30 p-3 rounded-lg border border-zinc-800/50">
            ...
            <div className="relative w-24 h-24 shrink-0 flex items-center justify-center">
              <svg className="w-full h-full -rotate-90" aria-label={`Weight gauge displaying ${weightKg} kg`}>
                <circle cx="48" cy="48" r="42" className="fill-none stroke-zinc-800" strokeWidth="6" />
                <circle
                  cx="48"
                  cy="48"
                  r="42"
              ...
              <div className="absolute flex flex-col items-center">
                 <span className={`text-xl font-bold font-mono ${statusColor}`}>{weightKg}</span>
                 <span className="text-[10px] text-zinc-500 uppercase">kg</span>
              </div>
            </div>
            
            <div className="flex flex-col justify-center flex-1">
               <span className="text-zinc-400 text-xs font-medium">Target: 2.50 kg</span>
               <div className="h-2 w-full bg-zinc-800 rounded-full mt-1.5 overflow-hidden" role="progressbar" aria-valuenow={percent} aria-valuemin={0} aria-valuemax={100}>
                  <div className="h-full bg-zinc-600 rounded-full transition-all duration-500" style={{ width: `${percent}%` }} />
               </div>
               <span className="text-[10px] text-zinc-500 mt-1 font-mono">{percent.toFixed(0)}% of target reached</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="bg-zinc-900/30 p-3 rounded-lg border border-zinc-800/50">
             <div className="flex justify-between items-end mb-1">
                <span className="text-2xl font-bold text-zinc-200">{chicken.age} <span className="text-sm font-normal text-zinc-500">Days</span></span>
